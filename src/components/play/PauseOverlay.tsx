...
const KEYS: Array<{ combo: string; desc: string }> = [
  { combo: "WASD / 方向键", desc: "驾驶飞船或地表探测车" },
  { combo: "Space", desc: "互动或跳跃（任务阶段）" },
  { combo: "鼠标拖动", desc: "调整镜头与观察方向" },
  { combo: "ESC / P", desc: "暂停或继续任务" },
          ...
          >
            <div className="grid gap-6 md:grid-cols-[1.1fr_0.9fr]">
              <div>
                ...
                <h2 className="mt-2 font-display text-3xl font-semibold text-white md:text-4xl text-shimmer">
                  操作员，请下达指令
                </h2>
                <p className="mt-3 max-w-md text-sm leading-7 text-white/70">
                  当前航段已进入待命状态，遥测数据仍在持续记录。你可以继续推进任务，也可以返回主视图重新规划。
                </p>
                {sceneHint ? (
                  <div className="mt-4 rounded-2xl border border-amber-300/20 bg-amber-300/5 px-4 py-3 text-xs text-amber-100/82">
                    <span className="font-semibold tracking-wider">任务提示：</span>
                    {sceneHint}
                  </div>
                ) : null}
                ...
                <div className="mt-6 grid grid-cols-2 gap-3">
                  ...
                  <ActionButton label="重新开始" accent="amber" onClick={onRestart}>
                    从本星任务起点重来
                  </ActionButton>
                  <ActionButton
                    label={voiceEnabled ? "关闭语音" : "开启语音"}
                    accent="fuchsia"
                    onClick={onToggleVoice}
                  >
                    {voiceEnabled ? "语音播报：开启" : "语音播报：关闭"}
                  </ActionButton>
                  <ActionButton label="返回主视图" accent="rose" onClick={onBack}>
                    退出当前任务
                  </ActionButton>
                </div>
              </div>
              ...
              <div>
                ...
                <ul className="space-y-2">
                  {KEYS.map((item) => (
                    <li
                      key={item.combo}
                      className="flex items-start gap-3 rounded-2xl bg-white/4 px-3 py-2 text-xs text-white/72"
                    >
                      <kbd className="min-w-[120px] rounded-lg border border-white/10 bg-black/30 px-2 py-1 font-mono text-[10px] text-cyan-200">
                        {item.combo}
                      </kbd>
                      <span className="leading-5">{item.desc}</span>
                    </li>
                  ))}
                </ul>
                ...
                <div className="text-[11px] text-white/45">
                  本地会持续保存当前进度与最佳成绩，临时关闭页面不会影响你已经解锁的行星节点。
                </div>
              </div>
            </div>
          </motion.div>
