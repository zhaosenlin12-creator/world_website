...
type Orb = { x: number; y: number; r: number; collected: boolean; vy: number; phase: number };
type Spark = { x: number; y: number; vx: number; vy: number; life: number; color: string };
type Player = { x: number; y: number; vx: number; vy: number; onGround: boolean; dead: boolean; facing: 1 | -1; jumpHold: number };

// 8 ?????????: [platform count, spacing, hazard ratio, orb count]
...
export default function LandingPlatformer({ active, accent, groundColor, onCollect, onHazard, onComplete }: Props) {
  ...
  const camYRef = useRef(0); // ?????: ??????? worldY, camY ?????
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < ps.orbs; i++) {
      const p = plats[1 + i * 2];
      if (p) orbs.push({ x: p.x + p.w / 2, y: p.y - 40, r: 14, collected: false, vy: 0, phase: Math.random() * Math.PI * 2 });
    }
  ...
  const step = (dt: number) => {
    ...
    // ?? (?????????)
    if (k[" "] || k["w"] || k["arrowup"]) {
      if (p.onGround) { p.vy = JUMP; p.onGround = false; p.jumpHold = 0.15; }
  ...
  return (
    <AnimatePresence>
      {active && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-30 flex items-center justify-center pointer-events-auto" style={{ background: "radial-gradient(circle at center, rgba(2,1,10,0.55), rgba(2,1,10,0.85))" }}>
          <div className="relative">
            <canvas ref={canvasRef} width={W} height={H} className="rounded-2xl border border-cyan-400/30 shadow-2xl" style={{ maxWidth: "92vw", maxHeight: "82vh" }} />
            {/* HUD */}
            <div className="absolute top-3 left-3 right-3 flex items-start justify-between pointer-events-none">
              <div className="glass-strong rounded-lg px-3 py-1.5 text-[11px] font-mono">
                <div className="text-cyan-300 uppercase tracking-widest text-[9px] mb-0.5">????</div>
              ...
              <div className="glass-strong rounded-lg px-3 py-1.5 text-[11px] font-mono text-right">
                <div className="text-amber-300 uppercase tracking-widest text-[9px] mb-0.5">??</div>
            ...
            {showReady && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="text-center">
                  <div className="text-amber-300/80 text-xs tracking-[0.5em] uppercase mb-2">??????</div>
                  <div className="text-white/95 text-2xl font-display gradient-text">????</div>
                </div>
              </motion.div>
            )}
          </div>
